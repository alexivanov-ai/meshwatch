...
// Corroborating evidence that a device really is a GREE air conditioner,
// from data discovery actually observed: the OUI vendor, the DHCP/mDNS name,
// a matched model, or the device's own web page title. A port number on its
// own is never enough (see the port-7000 finding below).
function looksLikeGree(d) {
  const hay = [
    d.vendor, oui.vendor(d.mac), d.name, d.model,
    d.web_title || (d.web && d.web.title)
  ].filter(Boolean).join(" ");
  return /\bgree\b/i.test(hay);
}
...
function collect(devices, extras) {
  ...
  for (const d of list) {
    ...
    for (const p of openPortsOf(d)) {
      ...
      // Port 7000 answering is suggestive of a GREE air conditioner, not
      // proof of one: 7000 is also AFS3-fileserver, Azureus/Vuze and any
      // number of dev servers, and nothing here ever tried an actual
      // unauthenticated command. Only name the vendor when something else
      // about the device corroborates it; otherwise say what was observed
      // and frame the GREE reading as a possibility (Hard Rules 3 and 4).
      const hedgedGree = p.port === 7000 && !looksLikeGree(d);
      findings.push({
        ...
        title: hedgedGree
          ? d.name + " has an unidentified service on port 7000"
          : d.name + " has " + (p.label || ("port " + p.port)) + " open",
        detail: (p.proto || "tcp").toUpperCase() + " " + p.port +
          (p.inferred ? " (inferred from SSDP — not a fresh probe)" : " answered a probe from this PC.") +
          (p.port === 7000
            ? (hedgedGree
              ? " Port 7000 is what GREE air conditioners use for their keyless local control API — if this is a GREE unit, it may accept commands with no key. Other software uses this port too, so treat the identification as unconfirmed."
              : " GREE's local control API accepts commands with no key.")
            : ""),
        reference: hedgedGree ? "unverifiable - service on port 7000 not identified" : (p.port === 7000 ? "local exposure" : "local scan"),
        action: p.port === 7000 || p.port === 23 ? "Block internet access" : "Open device",
        estimated: !!p.inferred || hedgedGree
      });
    }
  }
  ...
  if (dnsNodes.length === 1) {
    ...
    findings.push({
      ...
      mac: d.mac, device: d.name, ip: d.ip,
      severity: "high",
      title: (d.name || "This DNS server") + " is your single point of DNS failure",
