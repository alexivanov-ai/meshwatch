...
// AdGuard Home has no API for "which account is the admin", so Meshwatch
// assumes the install default. Every failure message names it, because a
// renamed admin account otherwise fails with nothing but "check the
// password" and no hint that the username is the problem.
...
// "one saved secret" pattern) and logs in as ADMIN_USER.
async function login(host, password) {
  const r = await tryJson(host, "/control/login", {
    method: "POST",
    body: { name: ADMIN_USER, password },
...
async function stats() {
  ...
  if (!host) {
    return {
      available: false,
      reason: "Could not log in to AdGuard Home as \"" + ADMIN_USER + "\". Check that the password is right, that this is the account name AdGuard expects (Meshwatch always logs in as \"" + ADMIN_USER + "\"), and that this PC can open http://" + t.host + "/",
...
async function blockClient(ip, { blocked } = { blocked: true }) {
  ...
  const host = await ensureAuth();
  if (!host) {
    return { ok: false, reason: "AdGuard Home internet blocking needs a live API session. Add the password for its \"" + ADMIN_USER + "\" account in Preferences." };
  }
