...
//   3. SSDP / UPnP search
//   4. DHCP leases from the DNS service - API or SSH once credentials are saved
...
function mdnsBrowse(durationMs = 4000) {
  return new Promise((resolve) => {
    ...
    // Keep every distinct (ip, service type) pair — a device can advertise
    // several mDNS service types (e.g. _googlecast._tcp and _airplay._tcp)
    // and all of them are useful for the services list. Naming still wants a
    // single "best" hit per IP; run() derives that from this array by taking
    // the first-seen record for that IP (see the merge loop), which matches
    // this function's previous one-per-IP, first-hit-wins Map semantics.
    const found = [];
    const seen = new Set();
...
function ssdpSearch(durationMs = 4000) {
  return new Promise((resolve) => {
    ...
    // Same "keep every distinct (ip, ST) pair" approach as mdnsBrowse above.
    const found = [];
    const seen = new Set();
    client.on("response", (headers, _statusCode, rinfo) => {
      const ip = rinfo && rinfo.address;
      if (!ip || !isDiscoverableHost(ip)) return;
      const key = ip + "|" + (headers.ST || "");
      if (seen.has(key)) return;
      seen.add(key);
      found.push({
        ip,
...
// DNS/DHCP service or router, which may already know a hostname for the lease.
...
async function run({ onProgress } = {}) {
  ...
  report("dhcp", { note: "reading DHCP leases" });
  ...
  for (const entry of arp) {
    ...
    // Preserve naming behavior exactly: mdnsBrowse/ssdpSearch above now keep
    // every (ip, type) pair, but the OLD dedup kept only the first record
    // seen for an IP (Map "first hit wins"), regardless of whether it had a
    // name. mHits/sHits are in the same discovery order as before, so the
    // first entry per IP is byte-for-byte the same record the old Map would
    // have stored — mHits[0]/sHits[0] is the exact equivalent, not
    // "prefer a named hit" (which would change naming for devices whose
    // first-seen mDNS record happened to lack a name).
    const m = mHits[0];
    const s = sHits[0];
    ...
    byMac.set(entry.mac, {
      mac: entry.mac,
      ip: entry.ip,
      lease, mdnsHit: m, ssdpHit: s, mdnsHits: mHits, ssdpHits: sHits,
  ...
  for (const d of devices) {
    ...
    d.services = svc.filter((x) => {
      const k = x.source + x.type;
  ...
  for (const d of devices) {
    ...
    delete d.mdnsHits; delete d.ssdpHits;
